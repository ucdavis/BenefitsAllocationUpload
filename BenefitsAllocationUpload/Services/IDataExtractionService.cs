...
namespace BenefitsAllocationUpload.Services
{
    public interface IDataExtractionService
    {
        ...
        string CreateFile(string fiscalYear, string fiscalPeriod, string transDescription, string orgDocNumber, string orgRefId, string transDocNumberSequence, string orgId, bool useDaFIS);
    }
    ...
    public class DataExtractionService : IDataExtractionService
    {
        ...
        protected string GetFilenameForOrgId(string orgId)
        {
        ...
        public string CreateFile(string fiscalYear, string fiscalPeriod, string transDescription, string orgDocNumber, string orgRefId,
                                 string transDocNumberSequence, string orgId, bool useDaFIS)
        {
            _fileName = GetFilenameForOrgId(orgId);

            using (var context = new FISDataMartEntities())
            {
                var transactions =
                    from t in context.usp_GetBudgetAdjustmentUploadDataForOrg(fiscalYear, fiscalPeriod,
                        transDescription, orgDocNumber, orgRefId, transDocNumberSequence, orgId, null, useDaFIS, false)
                    select new
                    {
                        t.UNIV_FISCAL_YEAR,
                        t.FIN_COA_CD,
